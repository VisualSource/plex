...
type tokenReader struct {
	toks []css_tokenizer.Token
	pos  int
}

func newTokenReader(toks []css_tokenizer.Token) *tokenReader {
...
func (r *tokenReader) peek() css_tokenizer.Token {
	if r.atEnd() {
		return nil
	}
	return r.toks[r.pos]
...
func (r *tokenReader) peekN(n int) css_tokenizer.Token {
	i := r.pos + n
	if i < 0 || i >= len(r.toks) {
		return nil
	}
...
func (r *tokenReader) next() css_tokenizer.Token {
	t := r.peek()
	if t != nil {
		r.pos++
	}
...
func (r *tokenReader) skipWhitespace() {
	for !r.atEnd() && r.toks[r.pos].IsToken() == css_tokenizer.TokenId_Whitespace {
		r.pos++
	}
}
...
func (r *tokenReader) peekIsWhitespace() bool {
	t := r.peek()
	return t != nil && t.IsToken() == css_tokenizer.TokenId_Whitespace
}
